...
impl BffApi<ServiceContext> for BackendApi {
    async fn api_registration_kyc_documents_post(
        ...
        Ok(ApiRegistrationKycDocumentsPostResponse::UploadURLCreatedSuccessfully(
            dto.into(),
        ))
    }
    ...
    async fn api_registration_kyc_informations_patch(
        ...
        self.state.invalidate_bff_cache(&user_id);
        Ok(ApiRegistrationKycInformationsPatchResponse::KYCInformationUpdated(
            dto.into(),
        ))
    }
    ...
    async fn api_registration_kyc_status_get(
        ...
        if use_default_cache {
            if let Some(cached) = self.state.get_kyc_status_cache(&user_id) {
                return Ok(ApiRegistrationKycStatusGetResponse::KYCStatusInformation(cached));
            }
        }
    ...
    async fn api_registration_limits_get(
        ...
        if let Some(cached) = self.state.get_limits_cache(&user_id) {
            return Ok(ApiRegistrationLimitsGetResponse::LimitsAndUsageDetails(cached));
        }
    ...
    async fn enrollment_bind(
        ...
        if let Some((device_record_id, bound_user_id)) = existing {
            if bound_user_id != req.user_id {
                return Ok(EnrollmentBindResponse::DeviceAlreadyBoundToADifferentUser(kc_error(
                    "CONFLICT",
