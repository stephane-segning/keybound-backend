...
fn verify_rsa(jwk: &Jwk, payload: &str, signature: &[u8]) -> Result<()> {
    let n_str = jwk.n.as_ref().ok_or_else(|| Error::unauthorized("Missing n for RSA"))?;
    let e_str = jwk.e.as_ref().ok_or_else(|| Error::unauthorized("Missing e for RSA"))?;

    let n = decode_base64_url(n_str)?;
    let e = decode_base64_url(e_str)?;

    let n_bn = BigNum::from_slice(&n)
        .map_err(|_| Error::unauthorized("Invalid RSA modulus"))?;
    let e_bn = BigNum::from_slice(&e)
        .map_err(|_| Error::unauthorized("Invalid RSA exponent"))?;

    let rsa = Rsa::from_public_components(n_bn, e_bn)
        .map_err(|_| Error::unauthorized("Failed to construct RSA key"))?;
        
    let pkey = PKey::from_rsa(rsa)
        .map_err(|_| Error::unauthorized("Failed to construct PKey"))?;
...
fn verify_ec(jwk: &Jwk, payload: &str, signature: &[u8]) -> Result<()> {
    ...
    if crv != "P-256" {
        return Err(Error::unauthorized(&format!("Unsupported EC curve: {}", crv)));
    }

    let x_str = jwk.x.as_ref().ok_or_else(|| Error::unauthorized("Missing x for EC"))?;
    let y_str = jwk.y.as_ref().ok_or_else(|| Error::unauthorized("Missing y for EC"))?;
    ...
    let x_bn = BigNum::from_slice(&x)
        .map_err(|_| Error::unauthorized("Invalid EC x coordinate"))?;
    let y_bn = BigNum::from_slice(&y)
        .map_err(|_| Error::unauthorized("Invalid EC y coordinate"))?;
    ...
    let is_valid = sig.verify(&digest, &ec_key)
